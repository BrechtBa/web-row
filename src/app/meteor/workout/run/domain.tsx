...
export class MeteorWorkout {
  ...
  lastUpdateDate: Date;
  meteorTrace: Array<Array<number>>;
  velocityHistory: WorkoutVelocityHistory;
  started: boolean;
  finished: boolean;
  ...
  constructor(workout: MeteorWorkoutData, user: User) {
    ...
    this.velocityHistory = [{time: new TimeDelta(0), velocity: 0}]
    this.started = false;
    this.finished = false;
  ...
  start() {
    this.startDate = new Date(new Date().getTime() + 3000);
    ...
    if(this.startDate !== null){
      ...
      if(time.timeDeltaMs > this.totalDuration.timeDeltaMs + 2000 && ! this.finished) {
        ...
        workoutExecutionRepository.storeWorkoutExecution(
          WorkoutExecution.createMeteorWorkoutExecution(
      ...
      // update history
      if(this.velocityHistory.length === 0 || time.timeDeltaMs - this.velocityHistory[this.velocityHistory.length-1].time.timeDeltaMs > 1.){
        this.velocityHistory.push({time: time, velocity: instantaneousVelocity})
      }
    }
